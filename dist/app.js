...
export async function createApp() {
    ...
    function buildTools(gh, sa, defaultRepos) {
        return [
            {
                name: "cve_scan",
                ...
                async execute(args) {
                    const repos = args.repos ?? defaultRepos;
            ...
            },
            {
                ...
                async execute(args) {
                    const results = await intelligence.enrichBatch(args.cve_ids, process.env.NVD_API_KEY);
            ...
            },
            {
                ...
                async execute(args) {
                    const result = await state.update([{ id: args.id, repo: args.repo, status: args.status, skipReason: args.skip_reason }], sa);
                    return JSON.stringify(result, null, 2);
                },
            },
            {
                ...
                async execute(args) {
                    const result = await state.compact(sa, { retentionDays: args.retention_days ?? 30 });
                    return JSON.stringify(result, null, 2);
                },
            },
        ];
    ...
    return {
        ...
        async health() {
            try {
                ...
                if (!res.ok)
                    return { app: "@git-fabric/cve", status: "unavailable", details: { error: "Token invalid" } };
                ...
                return { app: "@git-fabric/cve", status: "degraded", details: { error: err.message } };
            }
        },
    };
}
