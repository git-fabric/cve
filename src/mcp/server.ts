...
import type { GitHubAdapter, StateAdapter, Severity } from "../types.js";
...
const TOOLS = [
  {
    name: "cve_scan",
    description:
      "Scan managed repos for vulnerable dependencies via GitHub Advisory Database. " +
  ...
  },
  {
    name: "cve_enrich",
    description:
      "Fetch enriched vulnerability details for a CVE ID from NVD. " +
  ...
  },
  {
    name: "cve_triage",
    description:
      "Process pending CVE queue entries: apply severity policy and open PRs. " +
  ...
  },
  {
    name: "cve_queue_list",
    description:
      "List CVE queue entries filtered by status and severity.",
    inputSchema: {
      type: "object" as const,
      properties: {
  ...
  },
  {
    ...
    description:
      "Queue health dashboard: totals by status and severity, oldest pending, top repos.",
    inputSchema: {
      type: "object" as const,
      properties: {},
    },
  },
  {
    ...
    description:
      "Manually update status of a CVE queue entry (e.g. skip with reason).",
    inputSchema: {
      type: "object" as const,
      properties: {
...
export async function startServer(
  ...
  const server = new Server(
    { name: "@git-fabric/cve", version: "0.1.0" },
    { capabilities: { tools: {} } },
  );
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
      case "cve_scan": {
        const repos = (args?.repos as string[]) ?? managedRepos;
      ...
      case "cve_batch": {
        const cveIds = args?.cve_ids as string[];
        const nvdKey = process.env.NVD_API_KEY;
      ...
      case "cve_queue_update": {
        const result = await state.update(
          [{
            ...
            status: args?.status as "pending" | "pr_opened" | "skipped" | "error",
            skipReason: args?.skip_reason as string | undefined,
          }],
          stateAdapter,
        );
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
