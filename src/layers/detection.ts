...
function parsePip(content: string): Record<string, string> {
...
function parseGo(content: string): Record<string, string> {
  const deps: Record<string, string> = {};
  for (const line of content.split("\n")) {
    const match =
      line.trim().match(/^require\s+(\S+)\s+(\S+)/) ||
...
function parseCargo(content: string): Record<string, string> {
  const deps: Record<string, string> = {};
  let inDeps = false;
  for (const line of content.split("\n")) {
...
function parseManifest(manifest: ManifestFile): Record<string, string> {
...
async function queryGhsa(
  ...
  const data = await res.json() as {
    data?: {
      securityVulnerabilities?: {
        nodes?: Array<{
          advisory: { ghsaId: string; cveId: string | null; summary: string; severity: string; cvss?: { score: number } };
...
export async function detect(
  ...
  const allFindings: CveQueueEntry[] = [];
  ...
  for (const repoFull of repos) {
    ...
    for (const { path, ecosystem } of MANIFEST_PATHS) {
      ...
      if (content) manifests.push({ path, ecosystem, content });
    }
